...
class NetworkDataType : public interface::IFrame {
 ...
 public:
  ...
  void SetFromNetwork(T value) {
    this->value_ = ::srp::core::converter::EndianessConverter::Convert(value);
  }
  void Set(const T& value) { this->value_ = value; }
  T Get() const { return this->value_; }
  T GetForNetwork() {
    return srp::core::converter::EndianessConverter::Convert(this->value_);
  }
  __attribute_noinline__ std::vector<std::uint8_t> GetBuffor() const override {
    T value_temp =
        srp::core::converter::EndianessConverter::Convert(this->value_);
  ...
  srp::core::ErrorCode SetBuffor(std::vector<std::uint8_t> data) override {
    ...
    if (data.size() != size) {
      return srp::core::kBadVariableSize;
    }
    std::copy(data.begin(), data.end(), help_array.begin());
    std::memcpy(&value_, &help_array, size);
    value_ = srp::core::converter::EndianessConverter::Convert(this->value_);
    return srp::core::kOk;
  }
};
