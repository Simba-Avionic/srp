...
simba::database::objects::AppElement Json_parser::LoadJson(
...
simba::database::objects::AppElement Json_parser::ParseJson(
    const nlohmann::json& data) {
  std::string name = data["name"];
  simba::database::objects::NetInterfaceElement buffer(
      data["service_id"], data["interface"]["ip"], data["interface"]["port"],
      data["interface"]["ipc"]);
  std::unordered_map<std::string, std::string> pub_methods =
      parsePubMethods(data);
  std::unordered_map<std::string, simba::database::objects::ReqEventElement>
      req_events = parseReqEvents(data);
  std::unordered_map<std::string, simba::database::objects::EventElement>
      pub_events = parsePubEvents(data);
  ...
  simba::database::objects::AppElement results(
      name, buffer, pub_methods, req_events, pub_events, db, conf);

  return results;
}

std::unordered_map<std::string, std::string> Json_parser::parsePubMethods(
...
Json_parser::parseReqEvents(const nlohmann::json& json_data) {
  ...
  for (const auto& event : req_events_array) {
    ...
    simba::database::objects::ReqEventElement event_element(event_id,
                                                            service_id);
...
Json_parser::parsePubEvents(const nlohmann::json& json_data) {
  ...
  for (auto it = json_data["pub_events"].begin();
       it != json_data["pub_events"].end(); ++it) {
    ...
    for (const auto& subscriber : event_data["subscribers"]) {
      ...
      simba::database::objects::ServiceElement service_element(service_id, ip,
                                                               port);
...
Json_parser::parseDb(const nlohmann::json& data) {
  std::unordered_map<std::string, simba::database::objects::MethodElement> db;
  ...
  for (auto it = data["db"].begin(); it != data["db"].end(); ++it) {
    ...
    simba::database::objects::ServiceElement service_element(service_id,
                                                             ip_address, port);
    simba::database::objects::MethodElement method_element(method_id,
                                                           service_element);
...
std::unordered_map<std::string, std::string> Json_parser::parseConfig(
    const nlohmann::json& json_data) {
  std::unordered_map<std::string, std::string> conf;

  for (auto it = json_data["conf"].begin(); it != json_data["conf"].end();
       ++it) {
    const std::string& key = it.key();
