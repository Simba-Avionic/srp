...
class DiDImpl : public ara::diag::GenericDiD {
 private:
  ...
  ara::core::Result<ara::diag::OperationOutput> Read() noexcept override {
    ara::diag::OperationOutput res{ara::com::Convert2Vector<uint16_t>::Conv(
        ara::com::EndianConvert<uint16_t>::Conv(
  ...
  ara::core::Result<void> Write(
    ...
    const auto val_16 = ara::com::Convert<uint16_t>::Conv(val);
    if (val_16.has_value()) {
      const auto res = state_con_->ChangeState(
          ara::com::EndianConvert<uint16_t>::Conv(val_16.value()));
      if (res == 0) {
        return {};
      } else {
