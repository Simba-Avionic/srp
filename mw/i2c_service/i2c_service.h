...
#include "mw/i2c_service/eeprom/service/eeprom.hpp"
namespace simba {
namespace mw {
class I2CService : public core::ApplicationMW {
 private:
    ...
    com::soc::IpcSocket eeprom_sock_;
    i2c::PCA9685 pca9685_{&i2c_mtx};
    i2c::Eeprom eeprom_{&i2c_mtx};
    ...
    const std::vector<std::uint8_t> data);
    void EepromRxCallback(const std::string& ip, const std::uint16_t& port,
    const std::vector<std::uint8_t> data);
