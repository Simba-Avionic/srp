...
INSTANTIATE_TEST_SUITE_P(GetAdcRawReadandVoltageTestParams, GetAdcRawReadandVoltageTest, ::testing::Values(
    ...
    // EOF from i2c_service (empty vector): must not crash
    std::make_tuple(0, std::optional<std::vector<uint8_t>>{std::vector<uint8_t>{}},
      std::optional<uint16_t>{}, std::optional<float>{}),
