...
void BlinkingService::setDiodeState(const uint8_t diode_pin_id,
                                    const uint8_t state) {
  std::cout << "setting diode with pin #" << (int) diode_pin_id << " to state " << (int) state << std::endl;
...
void BlinkingService::blinkingLoop(const std::stop_token &token) {
  ...
  while (!token.stop_requested()) {
    ...
    for (size_t i = 0; i < diode_pins.size(); i++) {
      auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(start - now);
      ...
      auto ms_next_toggle = std::chrono::duration_cast<std::chrono::milliseconds>(diode_next_toggle[i] - now);
...
BlinkingService::BlinkingService() :
    diode_pins({1,2}),
    diode_delays({std::chrono::milliseconds(100), std::chrono::milliseconds(333)}),
    diode_states(diode_pins.size(), 0),
    diode_next_toggle(diode_pins.size(), std::chrono::high_resolution_clock::now()),
