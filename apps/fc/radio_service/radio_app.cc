...
void RadioApp::TransmittingLoop(const std::stop_token& token) {
  ...
  auto send = [&](auto pack_func) {
    ...
    mavl_logger.LogDebug() << "send bytes: " << std::vector<uint8_t>(buffer, buffer + len);
  ...
  while (!token.stop_requested()) {
    ...
    mavl_logger.LogDebug() << "send full Mavlink packets";
        ...
        case MAVLINK_MSG_ID_SIMBA_GS_HEARTBEAT: {
          ...
          if ((values & SIMBA_GS_VENT_VALVE) != (event_data->GetActuatorStates() & SIMBA_GS_VENT_VALVE)) {
            if (!servo_service_handler) {
              mavl_logger.LogWarn() << "Servo service handler not ready for VENT_VALVE.";
            } else {
              mavl_logger.LogDebug() << "Changing VENT valve to "
          ...
          if ((values & SIMBA_GS_DUMP_VALVE) != (event_data->GetActuatorStates() & SIMBA_GS_DUMP_VALVE)) {
            if (!servo_service_handler) {
              mavl_logger.LogWarn() << "Servo service handler not ready for DUMP_VALVE.";
            } else {
              mavl_logger.LogDebug() << "Changing DUMP valve to "
...
someip_logger{ara::log::LoggingMenager::GetInstance()->CreateLogger("SOME", "", ara::log::LogLevel::kInfo)}
{
}
}  // namespace apps
}  // namespace srp
