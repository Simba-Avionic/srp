...
using DiagMethod = std::function<std::optional<std::vector<uint8_t>>(
...
class IDiagController {
 public:
  ...
  virtual std::optional<std::vector<uint8_t>> Read(const uint16_t service_id,
                                                   const uint8_t diag_id) = 0;
  virtual simba::core::ErrorCode Write(const uint16_t service_id,
                                       const uint8_t diag_id,
                                       const std::vector<uint8_t> payload) = 0;
  virtual std::optional<std::vector<uint8_t>> Job(
