...
std::vector<uint8_t> OtherJob::GetSubServiceId(const DiagRequest& request) const {
  return request.GetPayload(0, subservice_length_+1);
}

DiagResponse OtherJob::Method(const DiagRequest request) {
  const std::vector<uint8_t> payload = request.GetPayload(subservice_length_+1);
  DiagResponse res = Callback(std::move(payload));
...
OtherJob::OtherJob(const std::string instance, uint8_t subservice_length) : JobCommon(instance),subservice_length_{subservice_length_} {}
}  // namespace diag
}  // namespace simba
