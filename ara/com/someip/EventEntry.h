...
struct Convert2Vector<ara::com::someip::EventEntry> {
  static std::vector<uint8_t> Conv(const ara::com::someip::EventEntry& in) {
    ...
    if constexpr (std::endian::native == std::endian::big) {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint16_t>::Conv(in.counter);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    } else {
      const auto temp_v =
          ara::com::EndianConvert<std::uint16_t>::Conv(in.counter);
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint16_t>::Conv(temp_v);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    }
    if constexpr (std::endian::native == std::endian::big) {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint16_t>::Conv(in.eventgroup_id);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    } else {
      const auto temp_v =
          ara::com::EndianConvert<std::uint16_t>::Conv(in.eventgroup_id);
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint16_t>::Conv(temp_v);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    }
