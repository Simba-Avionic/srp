...
struct Convert2Vector<ara::com::someip::EndpointOption> {
  static std::vector<uint8_t> Conv(const ara::com::someip::EndpointOption& in) {
    ...
    if constexpr (std::endian::native == std::endian::big) {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint16_t>::Conv(in.length);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    } else {
      const auto temp_v =
          ara::com::EndianConvert<std::uint16_t>::Conv(in.length);
      const auto temp_r_v =
    ...
    {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint8_t>::Conv(in.reserved1);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    }
    if constexpr (std::endian::native == std::endian::big) {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint32_t>::Conv(in.ip);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    } else {
    ...
    {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint8_t>::Conv(in.reserved2);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    }
    {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint8_t>::Conv(in.protocol_type);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    }
    if constexpr (std::endian::native == std::endian::big) {
      const auto temp_r_v =
          ara::com::Convert2Vector<std::uint16_t>::Conv(in.port);
      out.insert(out.end(), temp_r_v.begin(), temp_r_v.end());
    } else {
