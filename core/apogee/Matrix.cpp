...
Matrix Matrix::operator+(const Matrix& other) const {
  if (rows != other.rows || columns != other.columns) {
    throw std::invalid_argument(
        "Cannot add matrices: both matrices must have the same dimensions.");
  }
...
Matrix Matrix::operator*(const Matrix& other) const {
  if (columns != other.rows) {
    throw std::invalid_argument(
        "Cannot multiply matrices: column count must equal row count.");
  }
...
Matrix Matrix::operator-(const Matrix& other) const {
  if (rows != other.rows || columns != other.columns) {
    throw std::invalid_argument(
        "Cannot substract matrices: both matrices must have the same "
        "dimensions.");
  }
  Matrix result(rows, columns);
