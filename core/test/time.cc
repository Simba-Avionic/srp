...
class TestWrapper : public srp::core::time::TimeChanger {
 public:
  std::string TestConvertVariableToCommand(srp::core::time::Date_t date) {
    return ConvertVariableToCommand(date);
  }
  std::optional<srp::core::time::Date_t> TestparseDate(const std::string& dateStr) {
...
class ParseDateTest : public
        ::testing::TestWithParam<std::pair<std::string, srp::core::time::Date_t>> {
};

TEST_P(ParseDateTest, ParseDate) {
    TestWrapper wrapper;
    const auto& param = GetParam();
    std::optional<srp::core::time::Date_t> date = param.second;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_pair("Sun Jun  2 21:40:23 UTC 2024",
                        srp::core::time::Date_t{srp::core::time::Date_t{2024, 6, 2, 21, 40, 23}})
    )
);
class ConvertVariableToCommandTest : public
                    ::testing::TestWithParam<std::pair<srp::core::time::Date_t, std::string>> {
};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_pair(srp::core::time::Date_t{2024, 6, 2, 21, 40, 23}, "date 060221402024.23"),
        std::make_pair(srp::core::time::Date_t{2020, 1, 3, 28, 41, 55}, "date 010328412020.55")
    )
);
